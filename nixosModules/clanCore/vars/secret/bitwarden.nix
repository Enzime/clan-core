...
in
{
  ...
  options.clan.core.vars.bitwarden = {
    secretLocation = lib.mkOption {
      type = lib.types.path;
      default = "/etc/secret-vars";
      description = ''
        Location where the tarball with the Bitwarden secrets will be uploaded to and the manifest.
      '';
    };
  };
  config = {
    clan.core.vars.settings.fileModule =
      lib.mkIf (config.clan.core.vars.settings.secretStore == "bitwarden")
        (
          file:
          lib.mkIf file.config.secret {
            path =
              if file.config.neededFor == "users" then
                ...
                "${config.clan.core.vars.bitwarden.secretLocation}/activation/${file.config.generatorName}/${file.config.name}"
    ...
    clan.core.vars.settings.secretModule = lib.mkIf (
      config.clan.core.vars.settings.secretStore == "bitwarden"
    ) "clan_cli.vars.secret_modules.bitwarden";

    system.activationScripts =
      lib.mkIf ((config.clan.core.vars.settings.secretStore == "bitwarden") && !useSystemdActivation)
        {
          setupUserSecrets = lib.mkIf userSecrets (
            lib.stringAfter
              ...
              ''
                ...
                ${installSecretTarball}/bin/install-secret-tarball ${config.clan.core.vars.bitwarden.secretLocation}/secrets_for_users.tar.gz /run/user-secrets
              ''
          ...
          setupSecrets = lib.mkIf normalSecrets (
            lib.stringAfter
              ...
              ''
                [ -e /run/current-system ] || echo setting up secrets...
                ${installSecretTarball}/bin/install-secret-tarball ${config.clan.core.vars.bitwarden.secretLocation}/secrets.tar.gz /run/secrets
              ''
            // lib.optionalAttrs (config.system ? dryActivationScript) {
              supportsDryActivation = true;
            }
    ...
    systemd.services =
      lib.mkIf ((config.clan.core.vars.settings.secretStore == "bitwarden") && useSystemdActivation)
        {
          bitwarden-install-user-secrets = lib.mkIf userSecrets {
          ...
          bitwarden-install-secrets = lib.mkIf normalSecrets {
